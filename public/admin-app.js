...
function uiAbout(){
  const d=data;const paras=Array.isArray(d.paragraphs)?d.paragraphs:[];
...
function uiSkills(){
  ...
  const cats=['Frontend','CMS','Tools','Other'];
  return `<div class="section-block"><div class="section-title">Skill Icons</div><div class="skill-grid">${icons.map((s,i)=>`<div class="skill-card" style="flex-wrap:wrap"><div class="color-dot" style="background:${esc(s.color||'#3b82f6')}"></div><div style="flex:1;min-width:120px"><input class="control" data-sk="${i}.name" value="${esc(s.name||'')}" placeholder="Skill name"><div class="grid2" style="margin-top:6px;gap:6px"><input class="control" data-sk="${i}.color" value="${esc(s.color||'#3b82f6')}" placeholder="#hex"><select class="control" data-sk="${i}.category">${cats.map(c=>`<option value="${c}" ${(s.category||'Other')===c?'selected':''}>${c}</option>`).join('')}</select></div></div><button type="button" class="btn danger sm" data-del-sk="${i}">×</button></div>`).join('')}</div><button type="button" class="btn sm" id="addSkill" style="margin-top:12px">+ Add Skill Icon</button></div>
  <div class="section-block"><div class="section-title">Other Tools</div><div class="chips">${tools.map((t,i)=>`<span class="chip">${esc(t)}<button type="button" data-rm-tool="${i}">×</button></span>`).join('')}</div><div class="chip-add"><input class="control" id="toolInput" placeholder="e.g. Kajabi"><button type="button" class="btn sm" id="addTool">Add</button></div></div>`;
}
...
function bindUI(){
  ...
  const addSkill=document.getElementById('addSkill');if(addSkill)addSkill.onclick=()=>{data.skillIcons.push({name:'New Skill',color:'#3b82f6',category:'Frontend'});markDirty();renderEditor()};

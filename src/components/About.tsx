...
import { PERSONAL_INFO, ABOUT, ABOUT_TRAITS } from "../data/portfolioData";
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-5 flex flex-col gap-5">
            <div className="rounded-2xl bg-[#121222] border border-[#c9a227]/30 p-6 sm:p-8 text-center relative corner-bracket always-active shadow-xl shadow-black/60">
              ...
              <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#c9a227]/10 border border-[#c9a227]/30 text-xs font-mono text-[#f0d060]">
                ...
                <span>{ABOUT.statusPill}</span>
              </div>
            </div>

            {/* Education from personal.json + response from about.json */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
              {education.map((edu) => (
                <div
                  key={edu.degree}
                  className="p-4 rounded-xl bg-[#121222] border border-white/10 hover:border-[#c9a227]/30 transition-all"
                >
                  <div className="flex items-center gap-2 text-gray-400 text-xs font-mono mb-1">
                    ...
                    <span>{edu.status === "Completed" ? "Education" : "Pursuing"}</span>
                  </div>
                  <div className="text-white font-bold text-sm">
                    {edu.degree} · {edu.period.replace(" – ", "–")}
                  </div>
                  <div className="text-gray-400 text-[11px]">{edu.name}</div>
                </div>
              ))}
              ...
              <div className="p-4 rounded-xl bg-[#121222] border border-white/10 hover:border-[#c9a227]/30 transition-all">
                ...
                <div className="text-white font-bold text-sm text-[#f0d060]">{ABOUT.responseTime}</div>
                <div className="text-gray-400 text-[11px]">{ABOUT.responseNote}</div>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-7 flex flex-col items-start">
            ...
            <h2 className="font-display font-extrabold text-3xl sm:text-4xl text-white tracking-tight leading-tight mb-6">
              {ABOUT.heading} <br />
              <span className="gold-gradient-text">{ABOUT.headingHighlight}</span>
            </h2>
            ...
            <div className="flex flex-wrap items-center gap-2.5">
              {ABOUT.languages.map((lang) => (
                <span
                  key={lang.label}
                  className="px-3.5 py-1.5 rounded-full bg-[#c9a227]/10 border border-[#c9a227]/30 text-xs font-mono text-[#f0d060]"
                >
                  {lang.label}
                </span>
              ))}

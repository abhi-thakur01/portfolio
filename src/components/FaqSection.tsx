...
import { FAQS, SECTIONS } from "../data/portfolioData";

export const FaqSection: React.FC = () => {
  const [openIdx, setOpenIdx] = useState<number | null>(0);
  const s = SECTIONS.faqs;
    ...
    <section className="py-20 relative bg-[#07070d]">
      <div className="max-w-4xl mx-auto px-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-center">
          {s.trustItems.map((item) => (
            <div
              key={item}
              className="p-4 rounded-xl bg-white/[0.02] border border-white/5 flex items-center justify-center gap-2 text-xs text-gray-300"
            >
